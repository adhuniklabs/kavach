...
The following examples demonstrate the complete IR workflow using a realistic DEX swap pattern -
illustrative of the rigor expected, not a template restricted to smart contracts. The same
Spec-IR/Code-IR/Alignment-IR/Divergence-Finding shape applies to an RFC parser, a billing engine,
or any other spec-vs-code comparison; only the domain vocabulary changes.
...
**Scenario:** Identifying that the critical slippage protection requirement is completely
missing. Note the `severity`/`cvss_vector`/`cvss_score`/`kavach_confidence` fields at the top -
these are the KAVACH-specific additions on top of the base divergence-finding shape (see
`SKILL.md` Phase 5); everything else is the original piolium divergence-finding contract.
...
reasoning: |
  ...
  CVSS reasoning (why AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:H/A:N -> 8.6 High, not the "unbounded funds
  loss = automatic Critical" default): the attacker manipulates price/integrity of a single user's
  trade outcome (I:H) with no auth and no user interaction, over the network, with low complexity
  - but it does not read confidential data (C:N), does not cross a tenant/privilege boundary in the
  Scope sense (S:U, since the victim already trusts the pool contract with their funds), and does
  not deny service (A:N). If this codebase also lets the same missing check corrupt shared pool
  state affecting other users beyond the immediate trade (not shown here), re-score with S:C and
  this becomes Critical - always re-check Scope against the actual blast radius, don't default to
  Critical just because money is involved.
...
**What this shows:**
- Complete divergence finding with a computed CVSS vector/score/band (not an ad hoc label)
...
- Explicit CVSS reasoning showing why Scope/Confidentiality/Availability landed where they did,
  instead of defaulting to "money is involved = Critical"
