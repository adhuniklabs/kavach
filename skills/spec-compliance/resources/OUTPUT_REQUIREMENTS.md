...
When performing spec-to-code compliance analysis, produce structured IR following the formats
demonstrated in [IR_EXAMPLES.md](IR_EXAMPLES.md).
...
- Each record MUST include: `id`, `spec_excerpt`, `source_section`, `source_document`,
  `semantic_type`, `normalized_form`, `confidence`
- MUST use YAML format matching Example 1
- MUST extract minimum 10 Spec-IR items for any non-trivial specification (5+ pages of
  documentation)
...
- Each record MUST include: `id`, `file`, `function`, `lines`, `visibility`, `modifiers`,
  `behavior` (preconditions, state_reads, state_writes, computations, external_calls, events,
  postconditions), `invariants_enforced`
- MUST use YAML format matching Example 2
- MUST document line numbers for ALL claims (every precondition, state read/write, computation,
  external call)
...
- Each record MUST include: `id`, `spec_ref`, `code_ref`, `spec_claim`, `code_behavior`,
  `match_type`, `confidence`, `reasoning`, `evidence`
- MUST classify using exactly one of: `full_match`, `partial_match`, `mismatch`,
  `missing_in_code`, `code_stronger_than_spec`, `code_weaker_than_spec`
...
- Every Spec-IR item MUST have a corresponding Alignment record (no gaps)

### Phase 5 - Divergence Findings (when applicable)
- MUST create a detailed finding for EVERY `mismatch`, `missing_in_code`, `partial_match`, or
  `code_weaker_than_spec`
- Each finding MUST include: `id`, `severity` (a real CVSS v3.1 band, never an independent label -
  see `SKILL.md` Phase 5), `cvss_vector`, `cvss_score`, `kavach_confidence`
  (`confirmed`/`suspected`), `title`, `spec_claim`, `code_finding`, `match_type`, `confidence`
  (the 0-1 alignment-certainty score, distinct from `kavach_confidence`), `reasoning`, `evidence`,
  `exploitability`, `remediation`
- MUST use YAML format matching Example 4
- MUST quantify impact with concrete numbers (not "could be exploited" but "attacker gains $X,
  victim loses $Y")
...
- MUST produce a structured report following the 16-section format defined in `SKILL.md` Phase 6
- MUST include all IR artifacts (Spec-IR, Code-IR, Alignment-IR, Divergence Findings)
- MUST provide a Full Alignment Matrix showing all spec->code mappings
- MUST quantify risk and prioritize remediations by CVSS-derived severity
...
- EVERY Spec-IR item has a corresponding Alignment record (complete matrix)
...
- EVERY finding at CVSS High or above has an exploit scenario with a concrete attack sequence
- Economic/impact quantified with concrete numbers ($X loss, Y% ROI, Z transactions/day, or the
  non-financial equivalent for the domain in question)
...
- `severity` is a real CVSS band with the vector shown, `kavach_confidence` is `confirmed` or
  `suspected` with a named runtime test if `suspected` - never a hand-picked tier with no vector
  behind it
...
- MUST use consistent field names across all records (e.g., `spec_excerpt` not
  `specification_text`)
...
- NEVER invent a CVE, advisory, or RFC clause you have not actually fetched and read
