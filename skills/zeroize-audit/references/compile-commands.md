...
This reference covers how to generate and use `compile_commands.json` for the zeroize-audit
IR/ASM analysis pipeline. Read this before running Stage 7 (IR comparison) or Stage 8 (assembly
analysis) in `SKILL.md`.
...
```bash
# Isolate CARGO_TARGET_DIR to a scratch dir so this doesn't collide with the project's real build:
CARGO_TARGET_DIR=.kavach/tmp/zeroize-workspace/cargo-ir cargo rustc -- --emit=llvm-ir -C opt-level=0
CARGO_TARGET_DIR=.kavach/tmp/zeroize-workspace/cargo-ir cargo rustc -- --emit=llvm-ir -C opt-level=2

# Assembly for Rust (use instead of emit_asm.sh):
CARGO_TARGET_DIR=.kavach/tmp/zeroize-workspace/cargo-asm cargo rustc -- --emit=asm -C opt-level=2
# Output: target/release/deps/*.s (under the isolated CARGO_TARGET_DIR)
```

Pass the resulting `.ll` and `.s` files directly to `diff_ir.sh` and to your own reading (there is
no bundled Rust assembly analyzer in this skill - read the patterns in
`rust-zeroization-patterns.md` Section C by hand).
...
# Stage 1: Extract build-relevant flags for the TU (as a bash array)
FLAGS=()
...
# Stage 2: Emit IR at each level in opt_levels (always include O0 as baseline)
...
# Stage 3: Diff across all levels - O1 is the diagnostic level for simple DSE;
...
# Stage 4: Emit assembly at O2 for register-spill and stack-retention analysis
bash {baseDir}/tools/emit_asm.sh \
  --src /path/to/src/crypto.c \
  --out "$WORKDIR/<tu_hash>.O2.s" --opt O2 -- "${FLAGS[@]}"

# Stage 5: Analyze assembly output
bash {baseDir}/tools/analyze_asm.sh \
  --asm "$WORKDIR/<tu_hash>.O2.s" --out "$WORKDIR/<tu_hash>.asm-analysis.json"

# Cleanup
rm -rf "$WORKDIR"/<tu_hash>.*
```

Refer to `ir-analysis.md` for how to interpret IR diffs and identify wipe elimination patterns.
...
Entries may point to generated files in the build directory (e.g., `build/generated/config.c`) that don't exist in a clean checkout. Run the build system to generate them before running analysis. Preflight (Stage 1 in `SKILL.md`) will catch this if trial compilation is attempted.
...
- **Assembly analysis**: adapt register patterns by target when reading the output:
