...
This reference documents vulnerability patterns detected during a zeroize-audit pass over Rust
code. Each entry includes: what the flaw is, which stage/technique detects it, severity, category,
a minimal Rust snippet showing the bug, and a recommended fix.

---

## Section A - Semantic Patterns (source/rustdoc-level, no compiler needed)

These patterns are detectable from source or rustdoc JSON without executing the compiler - read
trait impls, derives, and field types directly (Stage 2/6 in SKILL.md's workflow).
...
## Section B - Dangerous API Patterns (source-grep detectable)

These patterns are found by grepping Rust source for calls to APIs that prevent or bypass
zeroization. Detection confidence is stronger when the call appears within ~15 lines of a
sensitive name; weaker (still worth flagging, but as `suspected`) otherwise - see SKILL.md's
Confidence Gating for how proximity feeds the signal count.
...
These patterns are **invisible to source-level reading** but detectable by reading the emitted
MIR, LLVM IR, or assembly directly (Stages 7-9 in SKILL.md's workflow). Each entry explains why
source inspection is blind to it and what compiler artifact reveals the flaw.
...
**Detection**: Emit MIR (`cargo rustc -- --emit=mir`) and look for `closure_body: _captured_field = move _local` where the local matches a sensitive name pattern.
...
**Category**: `STACK_RETENTION` | **Severity**: high
...
**Category**: `REGISTER_SPILL` | **Severity**: medium
...
**Category**: `REGISTER_SPILL` | **Severity**: high
...
## Section D - Undetectable Patterns (document, don't claim coverage)

These patterns are **not reliably detectable** by source grep, rustdoc inspection, or single-TU
IR/ASM reading. When you notice one during an audit, report it as `suspected` with an honest note
about why static analysis can't fully prove it, rather than either ignoring it or overclaiming
confidence.
...
**Why hard to detect**: Struct-level inspection doesn't see call sites; a blanket `Arc::clone` grep pattern produces massive false positives without knowing the wrapped type. MIR/IR/ASM reading can show the drop-glue but can't statically verify that every Arc clone is dropped before a given boundary - that needs inter-procedural ownership/alias tracking this skill doesn't attempt. Flag it as `suspected` when you spot the shape and note the gap explicitly.
...
**Why hard to detect**: A stack-retention read only fires if NO zeroing occurs at all - it won't distinguish partial zeroing that skips padding. Checking this properly needs `rustc -Z print-type-sizes` output compared against the zeroed byte ranges in IR, which is beyond a quick read. Flag `#[repr(C)]` secret structs for manual layout review when you see them.
...
**Why hard to detect**: Struct/derive-focused scans routinely skip `static`/`const` items entirely. This one is actually easy to grep for directly (`static\s+\w+.*Sensitive`) - do so explicitly rather than relying on the struct-level heuristics above to catch it.
...
**Why hard to detect**: The compiler-generated coroutine struct type is not in the rustdoc index, and its `Drop` impl is generated drop-glue, not user code. You can detect secrets live at yield points (B10/C-MIR2) but verifying the *generated* Drop glue for the coroutine state machine itself requires reading raw MIR for an anonymous type - note it as an open question when the shape matches, rather than asserting confirmed either way.
...
**Why hard to detect**: A grep on `Cow` alone has an unacceptably high false-positive rate without knowing whether the held type is sensitive - most `Cow<'_, str>` usage is log messages, not secrets. Only flag this when the `Cow`'s inner type is one you've already identified as sensitive in Stage 2.

---

### D6 - `mem::swap` Moving Secret Bytes to Non-Zeroizing Location

**Category**: `SECRET_COPY` | **Gap type**: type-aware operand analysis
...
**Why hard to detect**: A bare `mem::swap` grep without knowing both operand types would flag every swap in the codebase. In MIR this is a pair of assignments - detectable if you check both operand types by hand, but there's no shortcut; read every `mem::swap` call site touching a sensitive object individually.
