...
If scan mode is "important only" (this skill's default), filter out medium-precision results with `security-severity` < 6.0 from the report. The suite includes all medium-precision security queries to let CodeQL evaluate them, but low-severity medium-precision findings are noise.
...
## Mapping SARIF into a KAVACH finding

`security-severity` is a hint, not the finding's `cvss_score`. Before handing a result back to the
calling domain agent, read the flagged `file:line`, then compute a real CVSS vector per
`severity-model.md` - factor in whether the sink is actually reachable by an attacker-controlled
input in *this* codebase (SARIF alone can't tell you that; you have to trace it).
