...
With no `--threat-model` flag, CodeQL uses `remote` only (the `default` group). This is correct for most web applications and APIs - and is this skill's default. Expanding beyond `remote` is useful when the application's trust boundary extends to local inputs; see the run-analysis workflow's Step 3c for the deterministic escalation rules this skill applies (zero-input - no prompting).
...
Multiple models can be combined. Each additional model expands the set of sources CodeQL considers tainted, increasing coverage but potentially increasing false positives. Start with the narrowest set that matches the application's actual threat model (per the recon-driven escalation rules), then expand only on explicit request.
