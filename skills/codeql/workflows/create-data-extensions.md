...
fd '(qlpack|codeql-pack)\.yml$' . --exclude '.kavach/tmp/*' --exclude 'codeql_*.db' | while read -r f; do
...
rg -l '^extensions:' --glob '*.yml' --glob '!.kavach/tmp/**' --glob '!codeql_*.db/**' | head -20

# 3. Installed model packs
codeql resolve qlpacks 2>/dev/null | grep -iE 'model|extension'
```

**If any found:** Report and finish. These will be picked up by the run-analysis workflow.
...
if [ -z "$DB_NAME" ]; then
  ...
  if [ ${#FOUND_DBS[@]} -eq 0 ]; then
    ...
    # Multiple databases - take the most recently created one, log the rest. No prompting.
    DB_NAME="${FOUND_DBS[0]}"
  fi
fi
...
Read both CSV files and note source types and sink kinds with counts in `$OUTPUT_DIR/build.log`.
...
**Exit:** Either no gaps found (report adequate coverage and finish) OR gaps identified and modeled (proceed to Step 4)
...
Model every identified gap by default - this is a zero-input skill, do not stop to ask which ones
to model. Only skip a gap if it clearly has no security relevance (e.g. a logging-only wrapper with
no taint-relevant argument).

---

### Step 4: Create Data Extension Files

**Entry:** Step 3 identified gaps
**Exit:** YAML extension files created in `$OUTPUT_DIR/extensions/` and deployed to `<lang>-all` ext/ directory

Generate YAML data extension files for the gaps identified in Step 3.
