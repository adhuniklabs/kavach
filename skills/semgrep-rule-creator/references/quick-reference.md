...
```yaml
rules:
  - id: rule-id          # Unique identifier (lowercase, hyphens; kavach-<lang>-<pattern> when landing in semgrep-kavach.yml)
    languages:                 # Target language(s)
      - python
    severity: HIGH             # LOW, MEDIUM, HIGH, CRITICAL when authoring standalone;
                                # ERROR/WARNING/INFO ONLY when landing in semgrep-kavach.yml (see SKILL.md)
...
1. **Wrong annotation line**: `ruleid:` must be on the line IMMEDIATELY BEFORE the finding. No
   other text or code
