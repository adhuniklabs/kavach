...
This reference provides a disciplined, repo-grounded prompt that produces AppSec-usable threat
models. Use it when you need a reliable output contract and a consistent process to assemble the
threat model output.
...
If `.kavach/recon.json` already exists for this repository, read it first - it already answers most of Objective 1 (languages, frameworks, datastores, auth, LLM providers, payment processors, cloud, IaC) so this summary pass should focus its budget on Objective 2 (entry points, trust boundaries, security layers).
```
