...
class TestEvents(unittest.TestCase):
    ...
    def test_emit_then_read_round_trips(self):
        ...
        events.emit(self.dir, "budget_check", phase="hunt", allowed=6)
...
class TestSpendLedger(unittest.TestCase):
    def setUp(self):
        self.dir = tempfile.mkdtemp()
        run = state.init_audit(self.dir, "balanced", ["intel", "hunt"])
    ...
    def test_charge_accumulates_tokens_and_dollars_per_phase(self):
        ...
        budget.charge(self.dir, self.audit_id, "hunt", 6, tokens_in=1000, tokens_out=200,
                      cost_usd=0.5)
        ledger = budget.charge(self.dir, self.audit_id, "hunt", 2, tokens_in=500,
        ...
        self.assertEqual(ledger["spend_by_phase"]["hunt"]["tokens_out"], 300)
    ...
    def test_charging_nothing_leaves_the_spend_columns_alone(self):
        ...
        budget.init_budget(self.dir, self.audit_id, "balanced")
        budget.charge(self.dir, self.audit_id, "intel", 1, cost_usd=0.4)
        ledger = budget.charge(self.dir, self.audit_id, "hunt", 1)
    ...
    def test_cost_ceiling_sheds_and_says_why(self):
        ...
        budget.charge(self.dir, self.audit_id, "intel", 1, cost_usd=1.25)
        decision = budget.check(self.dir, self.audit_id, "hunt", 8)
    ...
    def test_the_shed_is_recorded_for_the_reports_limits_section(self):
        budget.init_budget(self.dir, self.audit_id, "balanced", max_cost_usd=1.0)
        budget.charge(self.dir, self.audit_id, "intel", 1, cost_usd=2.0)
        budget.check(self.dir, self.audit_id, "hunt", 8)
    ...
    def test_zero_ceiling_is_unlimited_not_exhausted(self):
        ...
        budget.charge(self.dir, self.audit_id, "intel", 1, cost_usd=999.0)
        self.assertEqual(budget.check(self.dir, self.audit_id, "hunt", 8).allowed, 8)

    def test_show_reports_remaining_spend(self):
        budget.init_budget(self.dir, self.audit_id, "balanced", max_cost_usd=2.0)
        budget.charge(self.dir, self.audit_id, "intel", 1, cost_usd=0.5)
    ...
    def test_a_pre_spend_ledger_backfills_instead_of_crashing(self):
        ...
        ledger = budget.charge(self.dir, self.audit_id, "hunt", 1, cost_usd=0.25)
