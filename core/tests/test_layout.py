...
SUBAGENT_PHASES = [p for p in modes.PIPELINE
...
class TestLayout(unittest.TestCase):
    ...
    def test_header_names_the_path_result_glob_will_find(self):
        header = dispatch.build_runtime_header("deep", "hunt", self.dir, "/repo", [])
        named = header.split("audit root):\n")[1].splitlines()[0].strip()
        self._stub(named)
        self.assertEqual(glob.glob(dispatch.result_glob(self.dir, "hunt")), [named])

    def test_fanout_results_share_one_phase_dir(self):
        paths = [dispatch.result_path(self.dir, "hunt", "kavach-billing", index=i)
                 for i in range(1, 9)]
        for p in paths:
            self._stub(p)
        self.assertEqual(sorted(glob.glob(dispatch.result_glob(self.dir, "hunt"))),
                         sorted(paths))

    def test_result_glob_does_not_leak_across_phases(self):
        self._stub(dispatch.result_path(self.dir, "hunt", "kavach-sast"))
        self._stub(dispatch.result_path(self.dir, "authz", "kavach-api"))
        self.assertEqual(len(glob.glob(dispatch.result_glob(self.dir, "hunt"))), 1)
    ...
    def test_ingest_reads_a_result_written_at_result_path(self):
        ...
        path = dispatch.result_path(self.dir, "hunt", "kavach-sast", index=1)
        with open(path, "w", encoding="utf-8") as fh:
            json.dump({"findings": [f.to_dict()]}, fh)
        self.assertEqual(dispatch.ingest(self.dir, "hunt", path), (1, 0))
