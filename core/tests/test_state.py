...
class TestState(unittest.TestCase):
    ...
    def test_init_seeds_pending_phases(self):
        run = state.init_audit(self.dir, "balanced", ["BL1", "BL2"], repository="o/r")
        self.assertEqual(run.mode, "balanced")
        self.assertEqual(run.status, RunStatus.IN_PROGRESS)
        self.assertEqual(set(run.phases), {"BL1", "BL2"})
        self.assertEqual(run.phases["BL1"].status, PhaseStatus.PENDING)

    def test_state_persists_and_reloads(self):
        state.init_audit(self.dir, "balanced", ["BL1"], repository="o/r")
        reloaded = state.load_state(self.dir)
        self.assertEqual(len(reloaded.audits), 1)
        self.assertEqual(reloaded.audits[0].phases["BL1"].status, PhaseStatus.PENDING)

    def test_set_phase_complete_stamps_and_clears_retry(self):
        run = state.init_audit(self.dir, "balanced", ["BL1"], repository="o/r")
        state.set_phase_status(self.dir, run.audit_id, "BL1", PhaseStatus.FAILED,
                               attempt=2, last_error="boom")
        state.set_phase_status(self.dir, run.audit_id, "BL1", PhaseStatus.COMPLETE,
    ...
    def test_init_audit_ids_are_unique_and_isolated(self):
        r1 = state.init_audit(self.dir, "balanced", ["BL1"], repository="o/r")
        r2 = state.init_audit(self.dir, "balanced", ["BL1"], repository="o/r")
        self.assertNotEqual(r1.audit_id, r2.audit_id)
        state.set_phase_status(self.dir, r1.audit_id, "BL1", PhaseStatus.COMPLETE)
        audits = {a.audit_id: a for a in state.load_state(self.dir).audits}
        self.assertEqual(audits[r1.audit_id].phases["BL1"].status, PhaseStatus.COMPLETE)
        self.assertEqual(audits[r2.audit_id].phases["BL1"].status, PhaseStatus.PENDING)
    ...
    def test_complete_audit_picks_the_newest_in_progress_run(self):
        old = state.init_audit(self.dir, "balanced", ["BL1"], repository="o/r")
        new = state.init_audit(self.dir, "deep", ["DP1"], repository="o/r")
    ...
    def test_audit_record_carries_a_budget_block(self):
        run = state.init_audit(self.dir, "deep", ["DP1"], repository="o/r")
    ...
    def test_latest_resumable_prefers_in_progress_never_complete(self):
        r1 = state.init_audit(self.dir, "deep", ["DP1"], repository="o/r")
        state.set_phase_status(self.dir, r1.audit_id, "DP1", PhaseStatus.COMPLETE)
        ...
        r2 = state.init_audit(self.dir, "deep", ["DP1", "DP2"], repository="o/r")
