...
Per-technology attack catalog: known attack classes, how to detect each in code, and a manual
review checklist, organized by protocol/format/library/infrastructure domain. This is the
reference `kavach-intel` pulls from when building the attack-surface dossier, and the reference any
domain agent (`kavach-sast`, `kavach-api`, `kavach-crypto`, `kavach-llm`, `kavach-supply`,
`kavach-config`) pulls from directly when it recognizes one of these domains in the code it's
reading. A single project routinely triggers five or more of these - domain identification here is
exhaustive, not exclusive.

---

## Domain identification signals

Identify a domain below applies when any of the following show up during recon or while reading
the code:
...
| RFC/spec named in code, comments, or docs | source, docs | RFC 6749, RFC 7519, RFC 9110 |
| Security-sensitive library in dependencies | manifests, lockfiles | xmlsec, python-jose, openssl, bouncycastle, pyyaml, pickle |
| Transport, storage, or compute type in the stack | recon.json, architecture | WebSocket, gRPC, Kafka, Redis, S3, Lambda, Docker |
| Component role suggests protocol/format handling | file/class names | `SAMLValidator`, `OAuthHandler`, `TemplateEngine`, `PDFRenderer` |
...
Record the list of identified domains before diving into any one of them - this list is the map of
which sections below are relevant to this specific audit.

---

## Research action sequence

For each identified domain not already covered in full by the templates below, perform in order:

1. **Web search** - `"<domain> known attacks"`, `"<domain> security vulnerabilities"`,
   `"<domain> CVE analysis"`, `"<domain> implementation pitfalls"`.
2. **Recent-CVE / bypass-technique research** - surface anything published in the last few weeks
   that a static training cutoff would miss; new bypass techniques for well-known protocols surface
   constantly.
3. **MCP research tools** (best-effort, if available in this environment) - deeper technical
   research beyond a plain web search. Fall back to plain web search/fetch if unavailable.
4. **Build an attack taxonomy** - classify each discovered attack by attack class, prerequisites,
   detection strategy, scanner-detectability (yes/no/partial), and relevance to this project's
   actual implementation. Don't transcribe generic advisories - filter through what this codebase
   actually does.

**Budget discipline**: prioritize by relevance to the highest-risk flows you've already found -
research the domains that appear in dangerous data/control paths first. A focused checklist against
the domains that matter beats an exhaustive one against domains that barely apply. Quality over
quantity.
...
Record findings per identified domain under `.kavach/attack-surface/` (or fold directly into the
domain's `agent-<domain>.json` findings if you're a domain agent working this domain live, per
`finding-schema.md`):

```markdown
### Domain: <name>

**Identified via:** <signal - e.g., "RFC 7519 named in code, go-jose dependency">
...
- [ ] <concrete check tied to this project's actual implementation>

**Research sources used:** <web search / MCP research / this reference>
```

Cross-reference every hit against the checklists below before writing your own from scratch - most
domains are already fully cataloged here.
...
See `parser-differentials.md` for the full canonicalization/encoding bypass catalog this domain
draws on.
...
This is a direct instance of the key-theft kill chain (`attack-trees.md` §1) - any confirmed path
here is `kill_chain: steal-keys`.
...
This is `kavach-supply`'s home domain - the guarddog/trivy/pip-audit/npm-audit/osv findings from
`tool-catalog.md` are leads against exactly this checklist.
...
This is precisely where `vuln-class-applicability.md`'s "NOT_APPLICABLE outside home language"
guidance flips back to APPLICABLE - a Rust/Go/Python codebase is NOT_APPLICABLE for memory
corruption generally, but re-check this specific surface the moment an `unsafe`/FFI/ctypes block
...
This is `kavach-llm`'s home domain - cross-reference `domains/llm.md`'s full checklist and bias
matrix before treating this table as complete; this table is the attack catalog, that file is the
audit procedure.
...
This feeds `kavach-crypto`'s `encryption_tls_and_at_rest` control directly.

---

#### SAML (transport-adjacent duplicate - see Authentication & Authorization Protocols above for the full table)

Cross-check both the auth-protocol table above and the parser-differential XXE/roundtrip patterns
in `parser-differentials.md` §4 - SAML findings routinely sit at the intersection of both.
...
`kavach-crypto`'s home checklist - cross-reference `insecure-defaults.md`'s weak-crypto regex
catalog, which finds the same class of bug from the config/fallback angle.
...
## Notes for kavach-intel and every domain agent using this reference

- Domain identification is **exhaustive, not exclusive** - a single project routinely triggers five
  or more of the sections above. Triage which to go deep on by how central the domain is to the
  highest-risk data/control flows you've already mapped, not by how many domains you can list.
- **Budget discipline**: one round of web search plus (if available) one round of MCP research per
  domain is normally sufficient. Don't spend the whole audit re-researching what's already
  cataloged above - a focused, project-specific checklist beats an exhaustive generic one.
  Prioritize domains that appear in high-risk flows first.
- When deeper research tools aren't available, plain web search of the top 2-3 results is
  sufficient - this catalog plus a couple of current results almost always covers the domain.
- This catalog is the primary input for `vuln-class-applicability.md` (deciding whether a checklist
  item here applies to the actual stack in front of you when the relevant scanner is unavailable)
  and for spec/RFC gap analysis (`parser-differentials.md` §3, when the domain is a standardized
  protocol/format).
- Record which domains were identified and which research sources were used - this is what lets a
  later reviewer or the reconciler confirm coverage was systematic, not spot-checked.
